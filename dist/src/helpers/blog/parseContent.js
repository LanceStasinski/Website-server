...
const parseContent = (numContent, reqKeys, filesArr, body, next) => {
    const content = [];
    for (let i = 1; i <= Number(numContent); i++) {
        ...
        if (contentObj.type === "image" || contentObj.type === "imageUrl") {
            if (contentObj.alt === "") {
                const err = new http_error_1.default("Alternative text needed.", 422);
                next(err);
                return err;
            }
            if (contentObj.caption === "") {
                const err = new http_error_1.default("Image caption needed.", 422);
                next(err);
                return err;
            }
        }
        if (contentObj.type === "code" ||
            contentObj.type === "heading" ||
            contentObj.type === "paragraph" ||
            contentObj.type === "imageUrl") {
            if (contentObj.text === "") {
                const err = new http_error_1.default("Text content missing.", 422);
                next(err);
                return err;
            }
        }
        if (contentObj.type === "code" && contentObj.language === "") {
            const err = new http_error_1.default("Code language missing.", 422);
            next(err);
            return err;
        }
        if (contentObj.type === "image" && Object.keys(imgData).length === 0) {
            const err = new http_error_1.default("Image specified but not provided.", 422);
            next(err);
            return err;
        }
